...
export class LeaderboardService {
  ...
  async getLeaderboard(period: string, limit: number, currentUser?: User) {
    ...
    const items = await Promise.all(users.map(async (u, index) => {
      // Calculate solvedCount
      const solvedCount = await this.submissionRepo.createQueryBuilder('s')
    ...
    if (currentUser) {
       ...
       const topRank = await this.userRepo.createQueryBuilder('u')
    ...
    return {
      items,
      currentUser: currentUser ? { rank: currentRank, score: currentUser.rating } : null,
    };
  }
}
