...
export class RoomCleanupService implements OnModuleInit {
  ...
  onModuleInit() {
    const minutes = this.configService.get<number>('ROOM_CLEANUP_INTERVAL_MINUTES', 10);
  ...
  async handleCleanup() {
    ...
    const now = Date.now();
    const gracePeriodMs = this.configService.get<number>('ROOM_CLEANUP_GRACE_PERIOD_MS', 5 * 60 * 1000); // 5 minutes default
    ...
    try {
      ...
      if (removedParticipantsCount > 0) {
        this.logger.log(`Cleaned up ${removedParticipantsCount} phantom participants.`);
      }

      // 2. Clear out ENDED sessions
      const endedSessions = await this.sessionRepo.find({ where: { status: 'ENDED' } });
      ...
      const activeSessions = await this.sessionRepo.find({ where: { status: 'ACTIVE' } });
      ...
      for (const room of rooms) {
        const pCount = await this.participantRepo.count({ where: { roomId: room.id } });
