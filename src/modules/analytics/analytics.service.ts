...
export class AnalyticsService {
  ...
  async getCourseAnalytics(courseId: string) {
    ...
    const stats = await this.submissionRepo.createQueryBuilder('s')
    ...
    return { 
      totalStudents, 
      avgCompletion: stats.users > 0 ? Math.round((stats.solved / (stats.users * 10)) * 100) : 0, // Giả sử trung bình 10 bài/khóa
