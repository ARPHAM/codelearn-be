...
export class ProblemService {
  constructor(
    ...
    @InjectRepository(ProblemVersion) private versionRepo: Repository<ProblemVersion>,
    @InjectRepository(Testcase) private testcaseRepo: Repository<Testcase>,
    @InjectRepository(ProblemLanguageFile) private langFileRepo: Repository<ProblemLanguageFile>,
  ...
  async create(dto: CreateProblemDto, user: User) {
    ...
    if (dto.testcases?.length) {
      const testcasesToSave = dto.testcases.map(tc => this.testcaseRepo.create({
        id: uuidv4(),
    ...
    if (dto.languageFiles?.length) {
      const langFilesToSave = dto.languageFiles.map(lf => this.langFileRepo.create({
        problem: savedProblem,
    ...
    if (dto.problemFiles?.length) {
      const pFilesToSave = dto.problemFiles.map(pf => this.fileRepo.create({
        id: uuidv4(),
        problemVersion: version,
        path: pf.path,
        content: pf.content,
    ...
    // Initialize stats
    const stats = this.statsRepo.create({ problemId: savedProblem.id, problem: savedProblem });
    await this.statsRepo.save(stats);

    return { problemId: savedProblem.id, versionId: version.id, slug: savedProblem.slug };
  }

  // Admin sees everything
  async findAllForAdmin() {
    const [items, total] = await this.problemRepo.findAndCount({ relations: ['createdBy'] });
  ...
  async findAllForLecturer(query: FilterProblemDto, user: User) {
    ...
    if (query.search) {
      qb.andWhere('(problem.title ILIKE :search OR problem.slug ILIKE :search)', { search: `%${query.search}%` });
    }
  ...
  async findOneForEdit(id: string, user: User) {
    ...
    if (!canEdit && problem.visibility !== 'PUBLIC') {
      throw new ForbiddenException('You can only view details of your own problems or public problems');
    }
    ...
    if (currentVersion) {
       testcases = await this.testcaseRepo.find({ where: { problemVersion: { id: currentVersion.id } }});
       problemFiles = await this.fileRepo.find({ where: { problemVersion: { id: currentVersion.id } }});
    }
    ...
    return { problem, versions, testcases, languageFiles, problemFiles, canEdit };
  }

  // Lecturer updates a problem (creates a new PENDING version)
  async update(id: string, dto: UpdateProblemDto, user: User) {
    const problem = await this.problemRepo.findOne({ where: { id }, relations: ['createdBy'] });
    if (!problem) throw new NotFoundException('Problem not found');
    ...
    const versionCount = await this.versionRepo.count({ where: { problem: { id } } });
    ...
    return { message: 'Problem updated successfully and pending approval', problemId: problem.id, versionId: version.id };
  }

  // View for Student (No hidden testcases, no solution code)
  async findOneForStudent(slug: string) {
    const problem = await this.problemRepo.findOne({ where: { slug, visibility: 'PUBLIC', status: 'ACTIVE' } });
    if (!problem) throw new NotFoundException('Problem not found or not public');

    const versionId = problem.currentVersionId;
    if (!versionId) throw new NotFoundException('Problem has no active version');
    ...
    const stats = await this.statsRepo.findOne({ where: { problemId: problem.id } });
    ...
    const problemFiles = await this.fileRepo.find({ where: { problemVersion: { id: versionId } } });
