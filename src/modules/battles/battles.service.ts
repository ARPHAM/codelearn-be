...
export class BattlesService {
  constructor(
    ...
    @InjectRepository(BattleSession) private battleRepo: Repository<BattleSession>,
  ...
  async challenge(dto: { opponentId: string; duration: number; topic: string }, player1: User) {
    const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
    if (!uuidRegex.test(dto.opponentId)) {
      throw new BadRequestException('ID đối thủ không hợp lệ (phải là UUID)');
    }
    const player2 = await this.userRepo.findOne({ where: { id: dto.opponentId } });
  ...
  async accept(id: string, player2: User) {
    ...
    if (!session) throw new NotFoundException('Yêu cầu thách đấu không tồn tại hoặc đã hết hạn');
    ...
    this.battlesGateway.broadcastMatchFound(session.id, {
      battleId: session.id,
      player1: session.player1Id,
      player2: session.player2Id,
    });

    this.battlesGateway.broadcastBattleStarted(session.id, {
      battleId: session.id,
  ...
  async cancel(id: string, user: User) {
    ...
    this.battlesGateway.broadcastBattleCancelled(id, 'Người chơi đã hủy trận đấu');
    return { success: true };
  }

  async updateProgress(battleId: string, userId: string, progress: number) {
    const session = await this.battleRepo.findOne({ where: { id: battleId, status: 'ACTIVE' } });
    if (!session) return;
    ...
    this.battlesGateway.broadcastProgress(battleId, {
      userId,
      progress,
    });
  }
  ...
  async getResult(id: string) {
    ...
    return {
      ...
      player1: { name: session.player1.fullName, progress: session.player1Progress },
      player2: { name: session.player2?.fullName, progress: session.player2Progress },
