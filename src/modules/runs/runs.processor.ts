...
export class RunsProcessor {
  ...
  async handleRunExecution(job: Job) {
    ...
    const memoryLmt = memoryLimit || language.defaultMemoryLimit || defaultMemory;
    ...
    try {
      ...
      try {
        ...
        if (internalExitCode === 124) {
          ...
          execResult.compileOutput = stderr || `Runtime Error (Exit Code: ${internalExitCode})`;
        } else {
      ...
      execResult.status = SubmissionStatus.RUNTIME_ERROR;
      console.error("[RunsProcessor] Global Error:", globalError);
    } finally {
      try {
