...
export class CreateLanguagesAndProblems1711785640000 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.createTable(new Table({
            ...
            uniques: [
                new TableUnique({ columnNames: ["exam_id", "problem_id"] })
            ]
        }), true);
        ...
        await queryRunner.createForeignKeys("problem_versions", [
            new TableForeignKey({ columnNames: ["problem_id"], referencedColumnNames: ["id"], referencedTableName: "problems", onDelete: "CASCADE" }),
            new TableForeignKey({ columnNames: ["created_by"], referencedColumnNames: ["id"], referencedTableName: "users", onDelete: "SET NULL" })
        ]);

        await queryRunner.createForeignKeys("problem_language_files", [
            new TableForeignKey({ columnNames: ["problem_id"], referencedColumnNames: ["id"], referencedTableName: "problems", onDelete: "CASCADE" }),
            new TableForeignKey({ columnNames: ["language_id"], referencedColumnNames: ["id"], referencedTableName: "languages", onDelete: "CASCADE" })
        ]);

        await queryRunner.createForeignKeys("testcases", [
            new TableForeignKey({ columnNames: ["problem_version_id"], referencedColumnNames: ["id"], referencedTableName: "problem_versions", onDelete: "CASCADE" })
        ]);

        await queryRunner.createForeignKeys("assignment_problems", [
            new TableForeignKey({ columnNames: ["assignment_id"], referencedColumnNames: ["id"], referencedTableName: "assignments", onDelete: "CASCADE" }),
            new TableForeignKey({ columnNames: ["problem_id"], referencedColumnNames: ["id"], referencedTableName: "problems", onDelete: "CASCADE" })
        ]);

        await queryRunner.createForeignKeys("bank_items", [
            new TableForeignKey({ columnNames: ["bank_id"], referencedColumnNames: ["id"], referencedTableName: "question_banks", onDelete: "CASCADE" }),
            new TableForeignKey({ columnNames: ["problem_id"], referencedColumnNames: ["id"], referencedTableName: "problems", onDelete: "CASCADE" })
        ]);

        await queryRunner.createForeignKeys("exam_problems", [
            new TableForeignKey({ columnNames: ["exam_id"], referencedColumnNames: ["id"], referencedTableName: "exams", onDelete: "CASCADE" }),
            new TableForeignKey({ columnNames: ["problem_id"], referencedColumnNames: ["id"], referencedTableName: "problems", onDelete: "CASCADE" })
        ]);
    }
