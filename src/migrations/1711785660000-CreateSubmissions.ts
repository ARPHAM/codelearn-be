...
export class CreateSubmissions1711785660000 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.createForeignKeys("submissions", [
            ...
            new TableForeignKey({ columnNames: ["language_id"], referencedColumnNames: ["id"], referencedTableName: "languages", onDelete: "SET NULL" })
        ]);

        await queryRunner.createForeignKeys("submission_files", [
            new TableForeignKey({ columnNames: ["submission_id"], referencedColumnNames: ["id"], referencedTableName: "submissions", onDelete: "CASCADE" })
        ]);

        await queryRunner.createForeignKeys("submission_results", [
            new TableForeignKey({ columnNames: ["submission_id"], referencedColumnNames: ["id"], referencedTableName: "submissions", onDelete: "CASCADE" }),
            new TableForeignKey({ columnNames: ["testcase_id"], referencedColumnNames: ["id"], referencedTableName: "testcases", onDelete: "CASCADE" })
        ]);
